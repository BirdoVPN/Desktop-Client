...
- [ ] **Windows, dual-stack node — IPv6 MTU (W11):** the pass criterion is the **MTU column** of `netsh interface ipv6 show subinterfaces`: the Birdo adapter row must show the WireGuard MTU (**1420** by default; `wireGuardMtu` in Settings if changed), not 1500 — before the F7 fix only the IPv4 row was written, so the v6 row stayed at 1500 and large v6 payloads blackholed. Then prove large v6 payloads actually flow through the tunnel: `curl -6 -o NUL https://speed.cloudflare.com/__down?bytes=10000000` must complete (10 MB of full-size v6 segments; it stalls/fails when the v6 MTU is wrong). Do **not** use `ping -f` for this: on Windows `-f` (Don't Fragment) is IPv4-only, so `ping -6 -l <n> -f` never exercises DF and tells you nothing about the v6 MTU.
