...
- **TLS certificate pinning** — API traffic pins the CA-chain SPKI (SHA-256)
  ...
  enforces every pin set against the vendored SSOT on each PR and, daily,
  checks that the live chain of every pinned host still matches — from the
  single vantage point a CI runner has, which for anycast hosts is evidence
  about one edge, not the host.
